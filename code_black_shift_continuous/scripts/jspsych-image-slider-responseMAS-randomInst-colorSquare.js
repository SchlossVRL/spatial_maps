...
jsPsych.plugins['image-slider-responseMAS-randomInst-colorSquare'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('image-slider-responseMAS-randomInst-colorSquare', 'stimulus', 'image');

  plugin.info = {
    name: 'image-slider-responseMAS-randomInst',
    description: '',
    parameters: {
      stimulus: {
  ...
  if (trial.prompt1 !== null){
    ...
    for (j = 0; j < 3; j ++){
      var shift = -15;//-75; 
      for (i = 0; i < 5; i++){
        shift = shift+ +25; //width: 3%; 
        html += '<div style="position: relative; left:'+shift+'px; width:50px;height: 50px; background-color:rgb('+trial.stimulus[i]+');"></div>'
//        html += '<img src="'+trial.stimulus[i+set]+'"; style ="width: 50px; position: relative; left:'+shift+'px;" '; 
     //   html += '"></img>'; 
      }
